...
# Run Configure will fail if it does not find ssl and crypto library
# So we give it a directory where they should be existing
if [ ! -d "$SCRIPTPATH/obj/local/arm64-v8a" ]; then
  echo "arm64-v8a ssl or crypto should have been generated"
  exit -1
fi
